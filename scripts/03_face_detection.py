...
from prop_score_ai.video_pipeline import process_face_features_dataset
from prop_score_ai.logging_utils import setup_logger


def main():
    parser = argparse.ArgumentParser(description="Run MediaPipe Face Mesh over extracted frames.")
    ...
    output_csv = Path(args.output_csv or (cfg["paths"]["processed_root"] / "face_features.csv"))
    sample_fps = args.sample_fps if args.sample_fps is not None else float(cfg["pipeline"]["frame_sample_fps"])
    logger = setup_logger("face_detection", cfg["paths"]["logs_root"] / "03_face_detection.log")
    process_face_features_dataset(video_root, frames_root, output_csv, class_names=cfg["labels"], sample_fps=sample_fps, logger=logger)
